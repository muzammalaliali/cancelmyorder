...
switch ($_GET['action']) {
    ...
    case 'listAllNicknames':
    {
        $query = $database->prepare("SELECT discord_id, steam_name FROM users");
        $query->execute();
        ...
        foreach ($results as $result) {
			...
			$new_name = trim(preg_replace('/[\pZ\pC]/u', '', $steam_name));
            $nicknames[$discord_id] = $new_name;
        }
    ...
    case 'getSteamBulk':
    {
        ...
        if (!isset($request_data['discord_ids'])) {
            http_response_code(400);
            echo json_encode('Missing request body');
            die();
        }
    
        $discord_ids = $request_data['discord_ids'];
        $placeholders = rtrim(str_repeat('?,', count($discord_ids)), ',');
        $query = $database->prepare("SELECT discord_id, steam_name FROM users WHERE discord_id IN ($placeholders)");
        $query->execute($discord_ids);
        ...
        foreach ($results as $result) {
            $discord_id = $result['discord_id'];
            $steam_name = $result['steam_name'];
            $nicknames[$discord_id] = $steam_name;
        }
        echo json_encode($nicknames);
        break;
    }
    
    case 'updateNitroBulk':
    {
        $request_body = file_get_contents('php://input');
        $request_data = json_decode($request_body, true);
        ...
        if (!is_array($request_data['discord_ids'])) {
            http_response_code(400);
            echo json_encode('Invalid request body format');
            die();
        }
        ...
        foreach ($ids as $id) {
            ...
            $query = $database->prepare("UPDATE users SET nitro = 1 WHERE discord_id = ?");
            if ($query->execute(array($id))) {
                array_push($statuses, array($id => "Success"));
            } else {
                array_push($statuses, array($id => $query->errorInfo()));
            }
        }
        echo json_encode(array("result" => $statuses));
        break;
    }
